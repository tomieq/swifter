...
struct MD5Digest: Hashable, RawRepresentable, LosslessStringConvertible, Codable {
    private let _digest_0: UInt64
    private let _digest_1: UInt64

    /// Perform hashing of the supplied data.
    init(from input: Data) {
    ...
    var rawValue: String { return self.description }

    var description: String {
        return String(format: "%016lx%016lx",
                      self._digest_0.byteSwapped,
                      self._digest_1.byteSwapped)
    }
...
fileprivate struct MD5State {
    ...
    init(_ message: Data) {
        ...
        let remainingByteCount: Int = self.feedFullChunks(in: message)
    ...
    var digest: (UInt64, UInt64) {
        let high = UInt64(a) | UInt64(self.b) << 32
        let low = UInt64(c) | UInt64(self.d) << 32
        return (high, low)
    }
    ...
    private mutating func feed(chunkPointer ptr: inout UnsafePointer<UInt32>) {
        ...
        self.feed(self.f0, ptr[00], 0xd76aa478, 07); self.feed(self.f0, ptr[01], 0xe8c7b756, 12)
        self.feed(self.f0, ptr[02], 0x242070db, 17); self.feed(self.f0, ptr[03], 0xc1bdceee, 22)
        self.feed(self.f0, ptr[04], 0xf57c0faf, 07); self.feed(self.f0, ptr[05], 0x4787c62a, 12)
        self.feed(self.f0, ptr[06], 0xa8304613, 17); self.feed(self.f0, ptr[07], 0xfd469501, 22)
        self.feed(self.f0, ptr[08], 0x698098d8, 07); self.feed(self.f0, ptr[09], 0x8b44f7af, 12)
        self.feed(self.f0, ptr[10], 0xffff5bb1, 17); self.feed(self.f0, ptr[11], 0x895cd7be, 22)
        self.feed(self.f0, ptr[12], 0x6b901122, 07); self.feed(self.f0, ptr[13], 0xfd987193, 12)
        self.feed(self.f0, ptr[14], 0xa679438e, 17); self.feed(self.f0, ptr[15], 0x49b40821, 22)

        self.feed(self.f1, ptr[01], 0xf61e2562, 05); self.feed(self.f1, ptr[06], 0xc040b340, 09)
        self.feed(self.f1, ptr[11], 0x265e5a51, 14); self.feed(self.f1, ptr[00], 0xe9b6c7aa, 20)
        self.feed(self.f1, ptr[05], 0xd62f105d, 05); self.feed(self.f1, ptr[10], 0x02441453, 09)
        self.feed(self.f1, ptr[15], 0xd8a1e681, 14); self.feed(self.f1, ptr[04], 0xe7d3fbc8, 20)
        self.feed(self.f1, ptr[09], 0x21e1cde6, 05); self.feed(self.f1, ptr[14], 0xc33707d6, 09)
        self.feed(self.f1, ptr[03], 0xf4d50d87, 14); self.feed(self.f1, ptr[08], 0x455a14ed, 20)
        self.feed(self.f1, ptr[13], 0xa9e3e905, 05); self.feed(self.f1, ptr[02], 0xfcefa3f8, 09)
        self.feed(self.f1, ptr[07], 0x676f02d9, 14); self.feed(self.f1, ptr[12], 0x8d2a4c8a, 20)

        self.feed(self.f2, ptr[05], 0xfffa3942, 04); self.feed(self.f2, ptr[08], 0x8771f681, 11)
        self.feed(self.f2, ptr[11], 0x6d9d6122, 16); self.feed(self.f2, ptr[14], 0xfde5380c, 23)
        self.feed(self.f2, ptr[01], 0xa4beea44, 04); self.feed(self.f2, ptr[04], 0x4bdecfa9, 11)
        self.feed(self.f2, ptr[07], 0xf6bb4b60, 16); self.feed(self.f2, ptr[10], 0xbebfbc70, 23)
        self.feed(self.f2, ptr[13], 0x289b7ec6, 04); self.feed(self.f2, ptr[00], 0xeaa127fa, 11)
        self.feed(self.f2, ptr[03], 0xd4ef3085, 16); self.feed(self.f2, ptr[06], 0x04881d05, 23)
        self.feed(self.f2, ptr[09], 0xd9d4d039, 04); self.feed(self.f2, ptr[12], 0xe6db99e5, 11)
        self.feed(self.f2, ptr[15], 0x1fa27cf8, 16); self.feed(self.f2, ptr[02], 0xc4ac5665, 23)

        self.feed(self.f3, ptr[00], 0xf4292244, 06); self.feed(self.f3, ptr[07], 0x432aff97, 10)
        self.feed(self.f3, ptr[14], 0xab9423a7, 15); self.feed(self.f3, ptr[05], 0xfc93a039, 21)
        self.feed(self.f3, ptr[12], 0x655b59c3, 06); self.feed(self.f3, ptr[03], 0x8f0ccc92, 10)
        self.feed(self.f3, ptr[10], 0xffeff47d, 15); self.feed(self.f3, ptr[01], 0x85845dd1, 21)
        self.feed(self.f3, ptr[08], 0x6fa87e4f, 06); self.feed(self.f3, ptr[15], 0xfe2ce6e0, 10)
        self.feed(self.f3, ptr[06], 0xa3014314, 15); self.feed(self.f3, ptr[13], 0x4e0811a1, 21)
        self.feed(self.f3, ptr[04], 0xf7537e82, 06); self.feed(self.f3, ptr[11], 0xbd3af235, 10)
        self.feed(self.f3, ptr[02], 0x2ad7d2bb, 15); self.feed(self.f3, ptr[09], 0xeb86d391, 21)

        (self.a, self.b, self.c, self.d) = (self.a &+ old.a, self.b &+ old.b, self.c &+ old.c, self.d &+ old.d)

        ptr = ptr.advanced(by: 16)
    }

    private var f0: UInt32 { return (self.b & self.c) | (~self.b & self.d) }
    private var f1: UInt32 { return (self.d & self.b) | (~self.d & self.c) }
    private var f2: UInt32 { return self.b ^ self.c ^ self.d }
    private var f3: UInt32 { return self.c ^ (self.b | ~self.d) }

    @inline(__always)
    private mutating func feed(_ f: UInt32, _ input: UInt32, _ magic: UInt32, _ shift: Int) {
        let s = self.a &+ input &+ magic &+ f
        let r = (s << shift) | (s >> (32 - shift))
        (self.a, self.b, self.c, self.d) = (self.d, self.b &+ r, self.b, self.c)
    }
}
